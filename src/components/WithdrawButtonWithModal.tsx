...
const WithdrawButtonWithModal: React.FC<WithdrawButtonWithModalProps> = ({
  ...
  const handleOpen = () => {
    if (typeof onClick === "function" && onClick() === false) return;
    setOpen(true);
    setErrorMsg(null);
    setSuccess(false);
    setAmount("");
  ...
  const isPersonalInfoComplete = (info?: {
    ...
    country?: string;
    gender?: string;
  }) => info && info.name && info.mobile && info.country && info.gender;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isPersonalInfoComplete(personalInfo)) {
      setOpen(false);
      onBlock?.("Please complete your personal information before requesting a withdrawal.");
      return;
    }

    if (!isAccountDetailsComplete(accountDetails)) {
      setOpen(false);
      onBlock?.("Please add your bank account details before requesting a withdrawal.");
      return;
    }

    const numAmount = Number(amount);

    if (numAmount < 1000 || numAmount > 100000) {
      setErrorMsg("Withdrawal amount must be between ₹1,000 and ₹1,00,000.");
      return;
    }
    ...
    setLoading(true);
    try {
      const data = await apiRequest<{ success: boolean; withdrawal?: Withdrawal; error?: string }>(
        "post",
        "/api/v1/user/withdrawal-request",
        { data: { amount: numAmount, remarks } }
      );
      if (!data.success) throw new Error(data.error || "Request failed");
      setSuccess(true);
      setTimeout(() => {
        ...
        {open && (
          ...
          >
            ...
            <motion.div
              ...
              transition={{ type: "spring", stiffness: 300, damping: 25 }}
            >
              {availableBalance !== undefined && availableBalance < 1000 ? (
                <>
                  <h2 className="text-xl font-bold mb-2">Insufficient Balance</h2>
                  <p className="text-yellow-700">
                    Your balance is below ₹1,000. Withdrawals are not allowed.
                  </p>
                  <button
                    className="mt-4 bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-semibold hover:bg-gray-300 transition"
                    onClick={handleClose}
                  >
                    Close
                  </button>
                ...
                <>
                  ...
                  <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <textarea
                      ...
                      rows={2}
                    />
                    <p className="text-sm text-gray-600">withdrawal amount: ₹1,000 - ₹1,00,000</p>
                    {error && <div className="text-red-500 text-sm">{error}</div>}
